...
const MASTER_CMS_SECTIONS = [
  ...
  // 6. BLOG
  {
    ...
    fieldsSchema: [
      ...
      { name: 'tags', label: 'Filter Tags (Comma Separated)', type: 'textarea', helperText: 'Comma-separated list of filter tags (e.g. All articles, a2 cow ghee, amla, avocado, turmeric)' },
  ...
  // 12. B2B / WHOLESALE & COMMERCIAL
  {
    sectionKey: 'b2b.hero',
    page: 'B2B (WHOLESALE)',
    title: 'B2B Hero Banner & Value Proposition',
    subtitle: 'India\'s Cleanest Food Ecosystem headline, commercial kitchen overview, and phone hotline.',
    fieldsSchema: [
      { name: 'tag', label: 'Top Small Tag', type: 'text' },
      { name: 'headline', label: 'Main Headline', type: 'text' },
      { name: 'highlightText', label: 'Headline Highlight', type: 'text' },
      { name: 'description', label: 'Commercial Overview Description', type: 'textarea' },
      { name: 'supportPhone', label: 'B2B Direct Line', type: 'text' }
    ],
    defaultFields: {
      tag: 'Nuva B2B & Modern Commercial Kitchens',
      headline: 'India’s Cleanest',
  ...
  },
  {
    ...
    subtitle: 'Head office, Anand processing unit, and retail store addresses.',
    fieldsSchema: [
...
async function seedMaster() {
  try {
    ...
    console.log('Connected to MongoDB. Seeding master website CMS content schemas...');
    ...
    console.log(`Successfully seeded ${MASTER_CMS_SECTIONS.length} master dynamic section schemas!`);
    process.exit(0);
  } catch (err) {
    console.error('Error seeding master sections:', err.message);
    process.exit(0);
  }
}
